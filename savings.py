...
class Savings:
    ...
    def savingsAllocations(self):
        binWid = self.years / (np.shape(self.alloc)[1] - 1)
        accounts = np.shape(self.alloc)[0]
        savingsAlloc = np.zeros((self.years,accounts))
        
        for n in range(self.years):
            for m in range(accounts):
                if n == 0:
                    savingsAlloc[n,m] = self.alloc[m][0]
                elif n > 0 and n < self.years:
                    curBin = math.floor(n / binWid)
                    savingsAlloc[n,m] = self.alloc[m][curBin] + (((n % binWid) / binWid) * (self.alloc[m][curBin+1] - self.alloc[m][curBin]))
                else:
                    savingsAlloc[n,m] = self.alloc[m][-1]
        
        savingsAlloc = savingsAlloc / 100
                
        return savingsAlloc
        
    def savingsContributions(self,savingsAlloc,earningsAlloc,netCash,totalExpenses,ret401,ageChild,baseSavings=None):
        accounts = np.shape(savingsAlloc)[1]
        savingsCont= np.zeros((self.years,accounts))
        savingsTotal = np.zeros((self.years,accounts))
    ...
    def investAllocations(self):
        accounts = np.shape(self.alloc)[0]
        earningsAlloc = np.zeros((self.years,accounts))
        ...
        for n in range(self.years):
            for m in range(accounts):
                if m == 0:
                    ...
                    mu = np.zeros((self.years,1))
                    sigma = np.zeros((self.years,1))
                    
                    muStart = 7.0
                    muEnd = 4.0
                    
                    mu[n] = muStart - ((n / self.years) * (muStart - muEnd))
                    sigma[n] = 0.35 * mu[n]
    ...
    def savingsCalc(self,netCash,totalExpenses):
        totalExpenses = np.sum(totalExpenses,axis=1).reshape((self.years,1))
        savings = np.zeros((self.years,1))
